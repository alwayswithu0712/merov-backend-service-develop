...
import { getBrandSchema } from '../schema/get-brand-schema';
...
export const GetBrandPagination = createParamDecorator((data, ctx): Pagination => {
    const req: RequestWithUser = ctx.switchToHttp().getRequest();

    const { error } = getBrandSchema.validate(req.query);
